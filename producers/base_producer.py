...
class BaseProducer(ABC):
    ...
    def __init__(
        ...
        Args:
            producer_name: 生产者名称
            trigger: APScheduler 触发器，由调用方根据配置构造并注入；
                当 ignore_schedule=True 时可为 None。
        ...
        try:
            ...
            if not self.ignore_schedule:
                if self.trigger is None:
                    raise ValueError(
                        f"[{self.producer_name}] 未注入 trigger 且未忽略调度，"
                        f"无法启动定时任务"
                    )
                ...
                logging.info(f"[{self.producer_name}] 使用触发器: {type(self.trigger).__name__}")
            else:
